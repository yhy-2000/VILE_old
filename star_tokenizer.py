...
try:
    from torch.hub import _get_torch_works

    torch_cache_works = _get_torch_works()
except ImportError:
    torch_cache_works = os.path.expanduser(
        os.getenv("TORCH_works", os.path.join(os.getenv("XDG_CACHE_works", "~/.cache"), "torch"))
    )
default_cache_path = os.path.join(torch_cache_works, "transformers")
